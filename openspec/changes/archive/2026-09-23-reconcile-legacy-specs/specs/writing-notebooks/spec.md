# writing-notebooks 变更增量

> 旧世界（双本子）对账：删除对象已死与已由别处承接的条款；R7–R11 保护类条款为本规格独有真相源，行为断言逐字保留、仅做宾语替换（「任一本子／草稿本／正文本」→「当前文档」；R11 宾语→「用户文档」）。

## MODIFIED Requirements

### Requirement: Leaving a project protects unsaved changes
系统 SHALL 在应用内操作即将卸载当前作品且当前文档的当前完整结构化文档不同于最后一次成功保存的完整结构化文档时，要求用户选择“保存并离开”“不保存并离开”或“取消”。未保存判断 MUST 包含只有标题、粗体、斜体或列表结构发生变化而可见纯文本未变化的情况。
...
**Reason**: 对象已消失——作品结构自 2026-08 起迁移为内容树（文件夹＋任意文档），草稿本／正文本双标签页 UI 与固定两本子不再存在，现实为内容树＋单一编辑器绑定当前文档。
**Migration**: 打开作品后的写作空间导航与当前文档切换由 `workspace-navigation` 承接；内容树的创建、打开与文档操作由 `content-tree-commands` 承接。

### Requirement: 两个本子的代码标识唯一对应
**Reason**: 对象已消失——前端 `draft`/`main` 本子代码标识已在全库消失（git 全历史对账零命中）；当前文档以稳定文档 ID 标识，不再存在「本子代码标识」这一概念对象。
**Migration**: 文档身份的现实真相源是内容树稳定文档 ID（见 `content-tree-commands`「前端可按文档 ID 读取单篇文档正文」）；「一个概念一个名字、不得引入第二英文名」的精神由 AGENTS.md 宪法第 5 条承接，不在规格内另立。

### Requirement: User can edit draft and main text
**Reason**: 删并——「两本」编辑框架已死；编辑能力与结构化事实源、格式往返语义已由 `structured-notebook-storage` 承接，保留将形成双真相源。
**Migration**: → `structured-notebook-storage`（带版本外层的 Tiptap JSON 事实源、schema 范围与读写边界校验）；「内部更新不产生用户内容」场景的精神（编辑器生命周期 transaction 不计入用户内容、不把未编辑作品标记为有未保存修改）已含于该规格现行表述，不随删除丢失。

### Requirement: Manual save writes both notebooks
**Reason**: 删并——「同时保存两本」对象已死；手动保存现为按受影响文档执行的事务性保存。
**Migration**: → `desktop-project-lifecycle`「User can save documents manually」条（一次手动保存是跨受影响文档正文文件与 `project.json` 的单一完整世代，事务暂存与崩溃恢复语义在该规格）。

### Requirement: Saved notebook content is loaded when reopening project
**Reason**: 删并——重开加载已按内容树文档 ID 读取文档正文，不再存在固定本子文件路径。
**Migration**: → 文档正文读取由 `content-tree-commands`「前端可按文档 ID 读取单篇文档正文」承接；重开作品回到上次编辑文档、当前文档切换由 `workspace-navigation`「记住上次编辑的文档」条承接。

### Requirement: Notebook switching preserves unsaved input
**Reason**: 对象已消失且行为已被相反行为取代——现行切换文档前先静默保存、失败阻止切换，不再「切换时各自保留未保存输入」。
**Migration**: → 本 change 在 `workspace-navigation` 新增的「切换文档先静默保存且失败阻止切换」条；旧行为已被相反行为取代，不迁移保留。
