...
Dependabot SHALL 以月度频率巡逻 npm 与 cargo 依赖，SHALL 按生态分组递单，MUST NOT 对 semver-major 大版本更新递单（大版本升级由用户手动主导）；每个生态在途 PR 数量 SHALL 受上限约束。

#### Scenario: 大版本更新不递单

- **WHEN** 某依赖发布新的 major 版本且 Dependabot 巡逻发现
- **THEN** Dependabot 不创建对应 PR

#### Scenario: 小版本更新按组递单

- **WHEN** 多个同生态依赖存在可用的非 major 更新
- **THEN** Dependabot 将它们合并为少量分组 PR 递单
- **AND** 在途 PR 总量不超过配置上限
