...
## Purpose
规定 AI 面板与 AI 编排前端代码的模块结构边界，防止逻辑回退为单文件巨石：状态迁移只经唯一外观进出、面板事件类型只有一份定义、编排聚焦模块不触碰 DOM、停靠区单向接收编排层动作。这些边界由巨石拆缝重构（change `extract-ai-logic-seams`，2026-09-21 归档）确立，供后续迭代（请求编排中心拆分、UI 全量更新等）持续依赖与自动校验。
