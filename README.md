...
Next Story 是一个带 AI 思考辅助的剧本写作桌面编辑器。当前已经完成只使用选区原文的最小 AI 思考闭环，带上下文的完整第一版 AI 闭环仍未实现。

用户在草稿本和正文本里亲手写作。AI 的位置是陪用户想、提问题和提供临时材料，不是替用户写，也不是自动改稿。当前可以从选区召唤 AI，并在独立面板查看临时思考材料；AI 不会直接改动两个本子。
...
| 已完成 | 最小 AI 思考闭环：在草稿本或正文本里选中文字召唤 AI，右侧面板显示纯文本临时思考材料（只使用选区原文，不带上下文） |
| 尚未完成 | 带上下文的完整第一版 AI 闭环：附近文本、整本摘要、多轮对话、会话历史、流式输出、AI 内容库 |
...
这是一个**有意收窄**的 AI 能力，用来验证“选中文字，让 AI 陪用户继续想”这一核心，不是完整的第一版 AI 闭环。

- 在草稿本或正文本里用鼠标或键盘选中至少包含一个非空白字符的文字后，选区附近出现“召唤 AI”入口。
- 点击入口时冻结一份选区快照（本子类型、原始文字、起止位置）；之后 AI 链路只依赖这份快照，不再读取编辑器。
- 首版只把用户选中的原文交给模型，由后端集中组装固定思考任务：围绕选区提出帮助创作者继续思考的问题，并给出几个思考方向。**不代写正文，不附带任何前后文、全文、摘要或知识库。**
...
1. 用户在草稿本或正文本选中文字并点击“召唤 AI”。前端只把选中的原文交给桥接层，不组装 Prompt，也不读取前后文。
2. `src/project-api.ts` 调用 `generate_ai_thinking` Tauri 桌面命令，只传选区原文。
3. 桌面命令从应用本地数据目录加载唯一保存的 LLM 配置，由 `src-tauri/src/llm_config/generate.rs` 集中组装固定首版思考任务并发起一次真实非流式 `chat/completions` 请求。
4. 合法非空的 assistant 回复作为纯文本返回前端，显示在独立 AI 面板；失败返回稳定 `code` 与中文可读 `message`，不含 API Key、Authorization、请求正文或完整远端响应。
5. 这条链路不持有任何写入草稿本或正文本的入口，AI 面板只显示临时材料。
