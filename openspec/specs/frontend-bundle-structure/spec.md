# frontend-bundle-structure Specification

## Purpose

定义前端生产构建产物的分包结构与单包体积约束：按依赖域静态分包（编辑器栈 / Tauri API / 应用代码），任一 JS 包保持在 500 kB 警戒线之下，超线时调整分包边界而非调高警告阈值。本规格防止功能与依赖增长时静默退回单包膨胀形态（审计 P2-14，队列 8e 立规）。
