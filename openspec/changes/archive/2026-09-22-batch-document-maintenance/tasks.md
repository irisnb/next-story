...
- [x] 5.1 delta 同步主规格：全部 delta（16 份，含守护句现代化与英文残留）落位；同步后 `openspec validate --specs` 55 份全过
  - 45 块落位（40 替换＋5 对 REMOVED/ADDED 原位换插）；主控独立复验 55 过、改动恰 16 份（同步中一次脚本缺陷被校验当场捕获，恢复基线重跑后零差异入库）
- [x] 5.2 全库术语终验收：`openspec/specs/` grep「临时对话」「当前对话」0 命中；裸「对话」命中全部落在保留清单（新建对话／对话框／恢复对话中／DSH 会话）；守护句处的「草稿本／正文本」已被「用户文档」泛化取代
  - 双零确认；裸「对话」仅 1 处＝resident-ai-session:9 DSH 运行时域合法引用；守护句残留 4 处均属 D7 登记规格或已裁决的旧格式路径守卫（llm-configuration:90）
- [x] 5.3 审计文档收尾：第八节 8d 行翻牌＋补记（含 D7 结构性失真登记为待办）；本 change 提交与归档（git-master 规范）
  - 8d 行翻「已完成并归档」、后续路线更新（下一项 8e）、补充十六落档；实现提交 `b363e0a`（74 文件）＋归档提交
