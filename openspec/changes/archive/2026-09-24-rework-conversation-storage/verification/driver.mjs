...
async function main() {
  ...
  if (cmd === "eval") {
    ...
    // clickAt <x> <y> — 按视口坐标点击（先由 eval 量取元素中心）
    await mouseClick(send, Number(args[0]), Number(args[1]));
    done({ clickedAt: [Number(args[0]), Number(args[1])] });
    ...
    const rect = await evaluate(send, RECT_EXPR(args[0]));
    if (!rect) die("悬停目标不存在或不可见: " + args[0]);
    await send("Input.dispatchMouseEvent", { type: "mouseMoved", x: rect.x, y: rect.y });
    done({ hovered: args[0], at: rect });
  } else if (cmd === "hoverClick") {
    // hoverClick <hover选择器> <click选择器> — 先悬停（触发悬停态 UI），再点第二目标
    const rect1 = await evaluate(send, RECT_EXPR(args[0]));
    if (!rect1) die("悬停目标不存在或不可见: " + args[0]);
    await send("Input.dispatchMouseEvent", { type: "mouseMoved", x: rect1.x, y: rect1.y });
    await new Promise((r) => setTimeout(r, 300));
    const rect2 = await evaluate(send, RECT_EXPR(args[1]));
    if (!rect2) die("点击目标不存在或不可见: " + args[1]);
    await mouseClick(send, rect2.x, rect2.y);
    done({ hover: args[0], click: args[1], at: rect2 });
  } else if (cmd === "type") {
    const rect = await evaluate(send, RECT_EXPR(args[0]));
