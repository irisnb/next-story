...
| 13 | 配置页只提示 API Key 会发送，未告知选区、方向和完整临时对话会发送给该服务 | 未修 | 建议 AI/LLM 隐私告知 change |
| 14 | README 与 AGENTS 把已经实现的“思维扩展”仍描述为未来功能 | 未修 | 建议文档同步 change，或随相关 AI change 一起修 |
...
建议 OpenSpec 范围：必须遵守产品边界：AI 输出仍是临时材料，AI 不能写回草稿本或正文本。这里处理的是“发送给第三方模型服务前，用户应知道哪些内容会离开本机”。
...
建议 OpenSpec 范围：这不是单纯润色。README、AGENTS、OpenSpec specs 之间必须同步，否则会影响后续 AI 助手和贡献者对产品边界的理解。
...
建议下一个 OpenSpec proposal 处理 issue 13、14：`sync-ai-privacy-and-docs`。

原因：当前剩余未修项都和 AI 数据流、隐私告知和文档同步有关。处理时要同时说明哪些创作内容会发送给第三方模型服务，并把 README、AGENTS 和正式规格对齐。
