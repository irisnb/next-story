# sidecar-dependency-hygiene Specification

## Purpose
定义 sidecar（DSH AI 引擎后台进程）运行时传递依赖的安全整备规则：有修复版的在册安全警报必须以范围形式钉修复版、无修复版的必须记录已接受风险与理由、DSH SDK 版本升级必须遵循文档化路径（`sidecar/UPGRADING.md`）并保持上述不变量。本规格保证依赖安全基线可长期维持，且安全钉版不构成 DSH SDK 版本升级的障碍。
